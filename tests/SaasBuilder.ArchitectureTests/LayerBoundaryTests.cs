...
namespace SaasBuilder.ArchitectureTests;

/// <summary>
/// Enforces layer-boundary invariants for all modules in the Modular SaaS SaasBuilder.
...
public sealed class LayerBoundaryTests
{
    ...
    // ── Test 4: All DbContexts inherit SaasBuilderDbContext ────────────────────────────────

    [Fact]
    public void All_DbContexts_inherit_SaasBuilderDbContext()
    {
        // Load-bearing: a DbContext that bypasses SaasBuilderDbContext loses the automatic tenant
        ...
        Type chassisDbContextType = typeof(SaasBuilderDbContext);

        // Collect all concrete DbContext subclasses from all Infrastructure assemblies.
        var nonSaasBuilderDbContexts = new List<string>();
        ...
        foreach ((string moduleName, Assembly infraAssembly) in InfrastructureAssemblies)
        {
            ...
            foreach (Type dbContextType in dbContextTypes)
            {
                ...
                {
                    nonSaasBuilderDbContexts.Add(
                        $"[{moduleName}] {dbContextType.FullName} does not inherit SaasBuilderDbContext");
                }
            }
        }

        nonSaasBuilderDbContexts.Should().BeEmpty(
            because: "Every module DbContext must inherit from SaasBuilderDbContext to receive the automatic " +
    ...
    /// it finds a directory containing a <c>SaasBuilder.sln</c> file.
    /// </summary>
    private static string FindRepoRoot()
    {
        ...
        {
            if (System.IO.File.Exists(System.IO.Path.Combine(dir, "SaasBuilder.sln")))
            {
                return dir;
            }
        ...
        throw new InvalidOperationException(
            "Cannot locate the repository root (directory containing SaasBuilder.sln) by walking " +
