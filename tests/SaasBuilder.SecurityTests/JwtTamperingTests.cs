...
/// <c>AddSaasBuilderAuthentication</c>. Disabling any one parameter (e.g. ValidateIssuer=false)
...
public sealed class JwtTamperingTests : IClassFixture<SaasBuilderSecurityFixture>
{
    private const string ProtectedEndpoint = "/api/v1/identity/me";

    private readonly SaasBuilderSecurityFixture _fixture;

    public JwtTamperingTests(SaasBuilderSecurityFixture fixture)
    {
    ...
    public async Task ValidToken_Returns200()
    {
        ...
        string token = SaasBuilderSecurityFixture.MintToken(
            tenantId: SaasBuilderSecurityFixture.TenantA,
            userId: SaasBuilderSecurityFixture.UserA);
    ...
    private static string BuildTamperedToken(string scenario)
    {
        return scenario switch
        {
            ...
            "missing-tenant-claim" => SaasBuilderSecurityFixture.MintTokenWithoutTenantClaim(
                SaasBuilderSecurityFixture.UserA),
