...
namespace SaasBuilder.IntegrationTests.Phase2.TestModules;

// ─────────────────────────────────────────────────────────────────────────────
// Test-only EF Core entity + DbContext
// These types exist ONLY to give the cross-tenant RLS E2E tests a real
// tenant-scoped table managed via EF Core + SaasBuilderDbContext global filters.
// They are never shipped to production.
// ─────────────────────────────────────────────────────────────────────────────

/// <summary>
/// Minimal tenant-scoped entity used in cross-tenant RLS E2E tests.
/// Implements <see cref="ITenantScoped"/> so the <see cref="SaasBuilderDbContext"/>
...
/// Inherits <see cref="SaasBuilderDbContext"/> to get automatic tenant query filters.
/// </summary>
...
public sealed class TenantScopedTestModule(string connectionString) : IModuleStartup
{
    ...
    {
        services.AddSaasBuilderPersistence<TestItemDbContext>(options =>
        {
