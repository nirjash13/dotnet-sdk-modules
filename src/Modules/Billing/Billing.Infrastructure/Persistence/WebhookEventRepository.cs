...
internal sealed class WebhookEventRepository(BillingDbContext db) : IWebhookEventRepository
{
    ...
    /// </remarks>
    public async Task<bool> TryRecordAsync(string idempotencyKey, string eventType, string providerName, CancellationToken ct)
    {
        ...
        int rowsAffected = await db.Database.ExecuteSqlAsync(
            ...
            VALUES ({idempotencyKey}, {eventType}, {providerName}, NOW() AT TIME ZONE 'UTC')
