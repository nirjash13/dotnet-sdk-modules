...
namespace SaasBuilder.Host.Observability;
...
/// </para>
/// <para>
/// Uses a raw Npgsql connection (from <c>ConnectionStrings:SaasBuilder</c>) to avoid circular
...
#pragma warning restore CA1031
    ...
    private async Task PollAsync(CancellationToken ct)
    {
        ...
        string? connectionString = config.GetConnectionString("SaasBuilder");
        if (string.IsNullOrWhiteSpace(connectionString))
        {
            _logger.LogDebug("OutboxLagReporter: ConnectionStrings:SaasBuilder not configured; skipping poll.");
            return;
        }
        ...
        if (lagResult is not DBNull && lagResult is not null)
        {
            ...
            SaasBuilderMeters.OutboxLagSeconds.Record(lagSeconds, tags);
